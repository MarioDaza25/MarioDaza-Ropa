...
public class EstadoController : BaseApiController
{
  ...
  var lista = _mapper.Map<List<EstadoDto>>(registros);
  ...
  [ProducesResponseType(StatusCodes.Status200OK)]
  [ProducesResponseType(StatusCodes.Status400BadRequest)]
  public async Task<ActionResult<IEnumerable<Estado>>> Post(EstadoDto nameDto)
  {
  var resultado = _mapper.Map<Estado>(nameDto);
  ...
  [ProducesResponseType(StatusCodes.Status400BadRequest)]
  public async Task<IActionResult> Update(int id, [FromBody] EstadoDto nameDto)
  {
      if (id != nameDto.Id)
      {
          return BadRequest();
